...
; Question Seven
; NOTE TO TA- my function is very close to working, I just have a issue where I get a procedure message on line 209 - otherwise the logic is there

(define (forAll start end)
(lambda (oper val)
  (define (my_iterator currNum total)
    (if (= currNum end)       ; previously was curr
        total
        (my_iterator (+ currNum 1)(oper currNum total) )))
    
  (my_iterator start 0)       ; changed to start from currNum
   ) 
  )

; Call with ((forAll 1 5) + 0)
