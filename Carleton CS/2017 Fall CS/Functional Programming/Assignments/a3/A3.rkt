...
(define (first trpl) ; returns the first element 
  (trpl 'x))

(define (second trpl) ; returns the secobd element 
  (trpl 'y))

(define (third trpl) ; returns the third element 
  (trpl 'z))
...
; 3a
(define (after L n)
  (cond  ; defines how to splice after a certain location
    ((null? L) L)
...
(define (append2 lis1 lis2)
  (cond ((null? lis1)
         lis2)  ; defines how to append two lists
        (else
         (cons (car lis1)
...
(if (= n 0)  (append2
  (append2
   (before L n)   ; defines how to splice with two  lists and a locationb
   A)
  (after L n)))
...
(define (multiDelete l index num l2)   ; deletes after a certain place
...
(define (depth lst)  ; checks the depth of the tree
  (if (not (pair? lst))
      0
...
(define (flattenList tree)  ; flattens tree
  (cond ((null? tree) tree)
...
(define-syntax cons-stream
    (syntax-rules () ; defines the counting process from a point n and creates a lst
...
(define (integers-start-from n)
  (cons-stream n (integers-start-from(+ n 1))))   ; defines the counting process from a point n and creates a lst
...
(define (lis->stream lis) ; defines the counting process from a point n and creates a lst
(cond ((null? lis) lis)
...
(define(stream->list str)
  (cond ((null? str) str) ; defines the counting process from a point n and creates a lst
...
(define (toInfinitynumbers n)
  (cons-stream n (toInfinitynumbers n))) ; defines the counting process from a point n and creates a lst
