...
//const API_KEY = '050b19a5161a443bc753a910256b7860'
 const API_KEY = 'b77a8db1a0cffb80e5d6f7db2ceadcc2' //f2f
...
function sendResponse(weatherData, res){
  var page = '<html><head><title>API Example</title></head>' +
    ...
    '</form>'
  if(weatherData){
    var weatherForCity = JSON.parse(weatherData);
    page += '<h1>Weather Info for ' + weatherForCity.name + ' </h1><p>' + weatherData + '</p>'
...
//Make an HTTP GET request to the openweathermap API
  let options = {
    host: 'http://food2fork.com/api/get',
    path: '/data/2.5/weather?q=' + city +   // city field
	'&appid=' + API_KEY
  }
...
http.createServer(function (req, res) {
  ...
  if (req.method == "GET"){
   // let reqData = ''
    ...
    var cityName  = query.substring(query.indexOf("=") + 1);
