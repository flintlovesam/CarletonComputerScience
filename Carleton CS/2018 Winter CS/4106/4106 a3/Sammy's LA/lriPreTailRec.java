...
static double totalOfSpots = 0;
  ...
  public static double strengthPopulator(int spot){  // returns +1/-1
   ...
   double functionF =  ((3 * (spot)/2) + rn.nextGaussian()) ;
...
if(strengthPopulator(rightSpot)>=5){
for(int k = 0; k<8;k++){
...
public static void main(String []args){
  ...
  otherWay(otherWay(otherWay(otherWay(other))));
}
...
public static double[] lriRec(int x, double[] state){
//code that affects the board
// --> then
  if(x==0){return state}
  else{
    return lriRec(x-1,board)
    }
}
